...
public class MapObject {
	...
	public MapObject(int zoom, int mapWidth, int mapHeight, LatLng center) {
		...
		System.out.println(centerPixel);
		init();
	}
	public void updateCurBounds() {
		...
		if(curBounds == null) {
			curBounds = new LatLngBounds(MapUtil.p2ll(pStart, maxZoom), MapUtil.p2ll(pEnd, maxZoom));
		} else {
			curBounds.setBounds(MapUtil.p2ll(pStart, maxZoom), MapUtil.p2ll(pEnd, maxZoom));
		}
	}
	...
	private void init() {
		len = MapUtil.getEndXYByZoom(zoom)+1;
		initTiles();
	}
